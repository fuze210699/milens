    ...
    for (const item of generatedFiles) {
      ...
      try {
        if (item.type === 'dir') {
          ...
          dirsRemoved++;
          autoRemoved.push({ file: item.path, action: `deleted directory (${item.count} files)`, preview: item.preview });
        } else {
          rmSync(fullPath, { force: true });
          filesRemoved++;
          autoRemoved.push({ file: item.path, action: 'deleted', preview: item.preview });
        }
      } catch {
        autoRemoved.push({ file: item.path, action: 'failed to delete (permission denied)', preview: item.preview });
      }
    }
...
function scanGeneratedFiles(rootPath: string): Array<{ path: string; type: string; count: number; preview?: string }> {
  const results: Array<{ path: string; type: string; count: number; preview?: string }> = [];
  ...
  for (const pattern of GEN_DIR_PATTERNS) {
    ...
    try {
      for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        if (isMilensGenerated(fullPath, entry.isDirectory())) {
          if (entry.isDirectory()) {
            ...
            try {
              const content = readFileSync(fullPath, 'utf-8').slice(0, 120);
              const firstLine = content.split('\n').find(l => l.trim() && !l.trim().startsWith('---') && !l.trim().startsWith('<!--'));
              if (firstLine) preview = firstLine.trim().slice(0, 80);
            } catch {}
            results.push({ path: relative(rootPath, fullPath).replace(/\\/g, '/'), type: 'file', count: 1, preview });
          }
        }
      }
...
function isMilensGenerated(filePath: string, isDir = false): boolean {
  const name = basename(filePath);
  if (name.startsWith('milens')) return true;
  if (name === 'milens.mdc') return true;

  try {
    let sniffPath = filePath;
    if (isDir) {
      // Directories can't be read as files — content-sniff a representative
      // child instead (e.g. SKILL.md), since a directory's own name is often
      // a domain name (e.g. "root", "backend") with no milens marker in it.
      const children = readdirSync(filePath, { withFileTypes: true });
      const repFile = children.find(c => c.isFile() && (c.name === 'SKILL.md' || c.name.endsWith('.md') || c.name.endsWith('.mdc')));
      if (!repFile) return false;
      sniffPath = join(filePath, repFile.name);
    }
    const content = readFileSync(sniffPath, 'utf-8').slice(0, 500);
    if (content.includes('name: milens') || content.includes('Auto-generated by milens')) {
      return true;
    }
    if (content.includes('generated-by: milens')) return true;
    if (content.includes('<!-- Generated by milens')) return true;
...
export function formatUninstallOutput(result: UninstallResult, dryRun: boolean, scanOnly: boolean): string {
  ...
  if (result.autoRemoved.length > 0) {
    ...
    for (const item of result.autoRemoved) {
      ...
      lines.push(`  ✗ ${item.file} — ${item.action}${preview}`);
    }
    lines.push('');
