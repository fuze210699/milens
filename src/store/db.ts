...
export class Database {
  ...
  isFileUpToDate(filePath: string, content: string): boolean {
    ...
    const row = this.db.prepare('SELECT hash FROM file_hashes WHERE path = ?').get(filePath) as any;
  ...
  insertSymbol(sym: CodeSymbol): void {
    this.db.prepare(
      `INSERT OR REPLACE INTO symbols (id, name, kind, file_path, start_line, end_line, exported, parent_id, signature)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(sym.id, sym.name, sym.kind, sym.filePath, sym.startLine, sym.endLine, sym.exported ? 1 : 0, sym.parentId ?? null, sym.signature ?? null);
  }
  ...
  insertLink(link: SymbolLink): void {
    this.db.prepare(
      `INSERT OR REPLACE INTO links (id, from_id, to_id, type, confidence, line_number)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).run(link.id, link.fromId, link.toId, link.type, link.confidence, link.line ?? null);
  }
  ...
  /**
   * Recursive upstream impact: find all symbols that transitively depend on target.
   * Uses recursive CTE — no need to load full graph into memory.
   */
  findUpstream(symbolId: string, maxDepth = 3): Array<{ symbol: CodeSymbol; depth: number; via: string }> {
    const rows = this.db.prepare(`
      WITH RECURSIVE upstream(id, depth, via) AS (
        ...
        FROM links l
        JOIN upstream u ON l.to_id = u.id
        WHERE u.depth < ? AND l.type IN ('calls', 'imports', 'extends', 'implements')
      )
      SELECT DISTINCT s.*, u.depth, u.via
      FROM upstream u
      JOIN symbols s ON s.id = u.id
      ORDER BY u.depth
    `).all(symbolId, maxDepth) as any[];
  ...
  /**
   * Recursive downstream: what does this symbol depend on?
   */
  findDownstream(symbolId: string, maxDepth = 3): Array<{ symbol: CodeSymbol; depth: number; via: string }> {
    const rows = this.db.prepare(`
      WITH RECURSIVE downstream(id, depth, via) AS (
        ...
        FROM links l
        JOIN downstream d ON l.from_id = d.id
        WHERE d.depth < ? AND l.type IN ('calls', 'imports', 'extends', 'implements')
      )
      SELECT DISTINCT s.*, d.depth, d.via
      FROM downstream d
  ...
  getStats(): { symbols: number; links: number; files: number } {
    ...
    const files = (this.db.prepare('SELECT COUNT(*) as c FROM file_hashes').get() as any).c;
