...
module.exports = (app) => {
  ...
  app.on('issue_comment.created', async (context) => {
    ...
    const titleMatch = body.match(/--title\s+"([^"]+)"/) || body.match(/--title\s+(\S.+)/);
    ...
    if (!branchMatch) {
      await context.octokit.issues.createComment({
        owner: repo.owner.login, repo: repo.name,
        issue_number: issue.number,
        body: `❌ Usage: \`/milens pr --branch <branch> --title "PR title" [--issue <num>] [--labels bug,feature]\`\n\nExample:\n\`/milens pr --branch fix/issue-123 --title "fix: resolve X" --issue 123\``,
      });
      return;
    }

    const branch = branchMatch[1];
    const title = titleMatch ? titleMatch[1] : `PR from @${actor}`;
    ...
    if (issueMatch) {
      try {
        linkedIssue = parseInt(issueMatch[1]);
        const issueData = await context.octokit.issues.get({
          owner: repo.owner.login, repo: repo.name,
          issue_number: linkedIssue,
        });
        const issueBody = issueData.data.body || '';
        const labelNames = issueData.data.labels.map(l => l.name);
        prBody = `Fixes #${linkedIssue}\n\n## Issue\n${issueBody}\n\n---\n\n## Goal\n<!-- Describe the goal of this PR -->\n\n## Changes Made\n<!-- List the changes -->\n\n## Verification\n<!-- How was this verified -->`;
        for (const lbl of labelNames) {
          if (!['bug', 'feature', 'security'].includes(lbl) && !labels.includes(lbl)) {
            labels.push(lbl);
          }
        }
      } catch {}
    ...
    try {
      ...
      if (linkedIssue) lines.push(`Linked: fixes #${linkedIssue}`);
      if (labels.length) lines.push(`Labels: ${labels.map(l => `\`${l}\``).join(', ')}`);

      lines.push('', '> Auto-generated by [Milens](https://github.com/fuze210699/milens)');
      ...
      if (err.message && err.message.includes('Reference already exists')) {
        msg = `Branch \`${branch}\` already has an open PR. Check the existing PR or use a different branch name.`;
      } else if (err.message && err.message.includes('does not exist')) {
        msg = `Branch \`${branch}\` not found. Make sure the branch exists on the remote.`;
      }
      await context.octokit.issues.createComment({
        owner: repo.owner.login, repo: repo.name,
        issue_number: issue.number,
