...
export const ALL_RULES: SecurityRule[] = [
...

  {
    ...
    patterns: [
      /(?:password|passwd|pwd)\s*[:=]\s*(?!\$\{|\w+\.|\w+\(|(?:string|number|boolean|any|unknown|object|void|never|bigint|symbol|null|undefined|true|false)\b)([A-Za-z0-9_\-/+!@#$%^&*]{4,}|['"`][^'"`\n]{4,}['"`])/im,
    ],
...

  {
    ...
    patterns: [
      /(?:secret|secretKey|secret_key|clientSecret)\s*[:=]\s*(?!\$\{|\w+\.|\w+\(|(?:string|number|boolean|any|unknown|object|void|never|bigint|symbol|null|undefined|true|false)\b)([A-Za-z0-9_\-+=/]{16,}|['"`][A-Za-z0-9_\-+=/]{16,}['"`])/i,
      /(?:signingKey|signing_key|encryptionKey|encryption_key)\s*[:=]\s*(?!\$\{|\w+\.|\w+\(|(?:string|number|boolean|any|unknown|object|void|never|bigint|symbol|null|undefined|true|false)\b)([A-Za-z0-9_\-+=/]{16,}|['"`][A-Za-z0-9_\-+=/]{16,}['"`])/i,
    ],
    excludeGlob: SECRETS_EXCLUDE,
...

  {
    ...
    patterns: [
      /(?:api[_-]?key|apiKey|apikey)\s*[:=]\s*(?!\$\{|\w+\.|\w+\(|(?:string|number|boolean|any|unknown|object|void|never|bigint|symbol|null|undefined|true|false)\b)([A-Za-z0-9_\-]{20,}|['"`][A-Za-z0-9_\-]{20,}['"`])/i,
      /(?:api[_-]?secret|apiSecret)\s*[:=]\s*(?!\$\{|\w+\.|\w+\(|(?:string|number|boolean|any|unknown|object|void|never|bigint|symbol|null|undefined|true|false)\b)([A-Za-z0-9_\-]{20,}|['"`][A-Za-z0-9_\-]{20,}['"`])/i,
    ],
    excludeGlob: SECRETS_EXCLUDE,
...

  {
    ...
    patterns: [
      /(?:token|authToken|accessToken|bearerToken)\s*[:=]\s*(?!\$\{|\w+\.|\w+\(|(?:string|number|boolean|any|unknown|object|void|never|bigint|symbol|null|undefined|true|false)\b)([A-Za-z0-9_\-+=.]{20,}|['"`][A-Za-z0-9_\-+=.]{20,}['"`])/i,
...

  {
    ...
    patterns: [
      /AUTH_TOKEN\s*=\s*(?!\$\{|\w+\.|\w+\()([^\s'"`\n]{8,}|['"`][^'"`\n]{8,}['"`])/i,
      /(?:\.env\s*(?:file)?\s*[:=]\s*(?!\$\{)([^\s'"`\n]+|['"`][^'"`]*['"`]))/i,
      /process\.env\.([A-Z_]+)\s*=\s*(?!\$\{|\w+\.|\w+\()([^\s'"`\n]{8,}|['"`][^'"`\n]{8,}['"`])/,
    ],
    excludeGlob: SECRETS_EXCLUDE,
  ...
  { id: 'SEC-051', category: 'secrets', owasp: 'A02:2021', severity: 'CRITICAL', name: 'Hardcoded JWT secret', description: 'JWT signing secret hardcoded in source code.', patterns: [/\b(?:jwtSecret|JWT_SECRET|jwt_secret)\s*[:=]\s*(?!\$\{|\w+\.|\w+\(|(?:string|number|boolean|any|unknown|object|void|never|bigint|symbol|null|undefined|true|false)\b)([A-Za-z0-9_\-]{16,}|['"`][A-Za-z0-9_\-]{16,}['"`])/, /secret\s*[:=]\s*(?!\$\{|\w+\.|\w+\(|(?:string|number|boolean|any|unknown|object|void|never|bigint|symbol|null|undefined|true|false)\b)([A-Za-z0-9_\-]{16,}|['"`][A-Za-z0-9_\-]{16,}['"`]).*jwt/i], excludeGlob: DEFAULT_EXCLUDE, fix: 'Use process.env.JWT_SECRET from environment variables.', confidence: 0.95, falsePositiveRisk: 'medium', enabled: true },
  { id: 'SEC-052', category: 'secrets', owasp: 'A02:2021', severity: 'CRITICAL', name: 'Hardcoded encryption key', description: 'Encryption key hardcoded in source code.', patterns: [/\b(?:encrypt(?:ion)?Key|ENCRYPT(?:ION)?_KEY|secretKey|SECRET_KEY)\s*[:=]\s*(?!\$\{|\w+\.|\w+\(|(?:string|number|boolean|any|unknown|object|void|never|bigint|symbol|null|undefined|true|false)\b)([A-Za-z0-9_\-]{16,}|['"`][A-Za-z0-9_\-]{16,}['"`])/i], fix: 'Use process.env.ENCRYPTION_KEY or a key management service.', confidence: 0.95, falsePositiveRisk: 'low', enabled: true },
  { id: 'SEC-053', category: 'secrets', owasp: 'A02:2021', severity: 'CRITICAL', name: 'Hardcoded AWS Secret Access Key', description: 'AWS secret access key exposed.', patterns: [/aws_secret_access_key\s*[:=]\s*(?!\$\{|\w+\.|\w+\()([A-Za-z0-9+\/=]{40}|['"`][A-Za-z0-9+\/=]{40}['"`])/i, /secretAccessKey\s*[:=]\s*(?!\$\{|\w+\.|\w+\()([A-Za-z0-9+\/=]{40}|['"`][A-Za-z0-9+\/=]{40}['"`])/], fix: 'Use AWS IAM roles or environment variables.', confidence: 0.95, falsePositiveRisk: 'low', enabled: true },
  { id: 'SEC-054', category: 'secrets', owasp: 'A02:2021', severity: 'CRITICAL', name: 'Hardcoded AWS Session Token', description: 'AWS session token exposed.', patterns: [/aws_session_token\s*[:=]\s*(?!\$\{|\w+\.|\w+\()([A-Za-z0-9+\/=]{100,}|['"`][A-Za-z0-9+\/=]{100,}['"`])/i], fix: 'Use temporary credentials via IAM roles.', confidence: 0.95, falsePositiveRisk: 'low', enabled: true },
  ...
  { id: 'SEC-065', category: 'secrets', owasp: 'A02:2021', severity: 'HIGH', name: 'Generic Refresh Token', description: 'OAuth refresh token hardcoded.', patterns: [/refresh[_-]?token\s*[:=]\s*(?!\$\{|\w+\.|\w+\(|(?:string|number|boolean|any|unknown|object|void|never|bigint|symbol|null|undefined|true|false)\b)([A-Za-z0-9_\-.]{16,}|['"`][A-Za-z0-9_\-.]{16,}['"`])/i], fix: 'Store refresh tokens in secure storage.', confidence: 0.85, falsePositiveRisk: 'low', enabled: true },
  ...
  { id: 'SEC-070', category: 'secrets', owasp: 'A02:2021', severity: 'HIGH', name: 'Hardcoded OAuth client secret', description: 'OAuth client secret hardcoded.', patterns: [/client[_-]?secret\s*[:=]\s*(?!\$\{|\w+\.|\w+\(|(?:string|number|boolean|any|unknown|object|void|never|bigint|symbol|null|undefined|true|false)\b)([A-Za-z0-9_\-]{16,}|['"`][A-Za-z0-9_\-]{16,}['"`])/i, /CLIENT_SECRET\s*=\s*(?!\$\{|\w+\.|\w+\()([A-Za-z0-9_\-]{16,}|['"`][A-Za-z0-9_\-]{16,}['"`])/], fix: 'Use environment variables or OAuth secret management.', confidence: 0.95, falsePositiveRisk: 'low', enabled: true },
  ...
  // ═══════════════════════════════════════════════════════════════
  { id: 'SEC-186', category: 'secrets', owasp: 'A02:2021', severity: 'HIGH', name: 'Hardcoded SMTP Credential', description: 'SMTP credentials hardcoded.', patterns: [/smtp.*(?:user|pass)\s*[:=]\s*(?!\$\{|\w+\.|\w+\()([^\s'"`\n]{3,}|['"`][^'"`\n]{3,}['"`])/i, /nodemailer.*auth\s*:\s*\{[^}]*pass\s*:\s*(?!\$\{|\w+\.|\w+\()([^\s'"`,\n]{3,}|['"`][^'"`\n]{3,}['"`])/i], fix: 'Use environment variables.', confidence: 0.95, falsePositiveRisk: 'low', enabled: true },
  { id: 'SEC-187', category: 'secrets', owasp: 'A02:2021', severity: 'HIGH', name: 'Hardcoded LDAP Credential', description: 'LDAP bind password.', patterns: [/ldap.*(?:password|bindpw)\s*[:=]\s*(?!\$\{|\w+\.|\w+\()([^\s'"`\n]{3,}|['"`][^'"`\n]{3,}['"`])/i], fix: 'Use environment variables.', confidence: 0.95, falsePositiveRisk: 'low', enabled: true },
  { id: 'SEC-188', category: 'secrets', owasp: 'A02:2021', severity: 'HIGH', name: 'Hardcoded FTP Credential', description: 'FTP password hardcoded.', patterns: [/ftp.*(?:password|pass)\s*[:=]\s*(?!\$\{|\w+\.|\w+\()([^\s'"`\n]{3,}|['"`][^'"`\n]{3,}['"`])/i], fix: 'Use environment variables.', confidence: 0.95, falsePositiveRisk: 'low', enabled: true },
